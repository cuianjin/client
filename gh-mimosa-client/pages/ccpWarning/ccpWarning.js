		...
		init: function() {
			...
			var createOptions = {
				"NUMBER": function(dataUnit) {
					...
					row.appendTo(form);
					var x0 = $('<div class="col-sm-8 col-xs-12"><div class="form-group"><div class="input-group"><input name="param0" type="text" class="form-control input-sm" placeholder="指标项描述" required maxlength="60">' + initDataUnit(dataUnit) + '</div></div></div>');
					x0.appendTo(row);
			...
			$('#dataTable').bootstrapTable({
				...
				columns: [{
					...
					align: 'center',
					formatter: function(val, row, index) {
						return row.showOptions ? util.table.formatter.convertRiskLevel(val) : '';
					}
				}, {
					align: 'center',
					formatter: function(val, row, index) {
...
//						};
//
//						return row.showWarning ? util.table.formatter.generateButton([updateButton, deleteButton], 'dataTable') : '';
