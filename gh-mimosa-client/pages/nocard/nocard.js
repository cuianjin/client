			...
			pagesInit: function() {
				...
				var noCardConfig = {
					ajax: function(origin) {
						http.post(util.buildQueryUrl(config.api.ope.noCard.list, {
							...
							registerTimeBegin: noCardOptions.registerTimeBegin,
							registerTimeEnd: noCardOptions.registerTimeEnd
						}), function(rlt) {
							origin.success(rlt)
						})
					},
				...
				$(document.noCardSearchForm.registerTimeBegin).val(tim);
				...
				function getNoCardQueryParams(val) {
					...
					noCardOptions.registerTimeBegin = form.registerTimeBegin.value.trim()
					noCardOptions.registerTimeEnd = form.registerTimeEnd.value.trim()
//					noCardOptions.registerTimeEnd = getNextDate(noCardOptions.registerTimeEnd)	// 结束时间加一天
					return val
				}
			...
			bindEvent: function() {
				...
				$("#approveBut").on('click', function() {
					...
					$('#approveForm').ajaxSubmit({
						url: config.api.ope.noCard.feedback,
