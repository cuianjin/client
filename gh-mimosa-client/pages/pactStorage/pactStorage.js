...
//						return util.table.formatter.generateButton(buttons, 'dataTable');
						var format = util.table.formatter.generateButton(buttons, 'dataTable');
						if( row.lifeState == 'OVER_VALUEDATE' || row.lifeState == 'OVER_VALUEDATE' && row.holdShare=='0' && row.applyAmount=='0'){
		            		format += '<span style=" margin:auto 0px auto 10px;cursor: pointer;" class="fa fa-trash-o item-invalid"></span>';
						}else{
							format += '<span style=" margin:auto 0px auto 20px;cursor: pointer;"></span>';
						}
		            	return format
					},
					events: {
						...
						},'click .item-transfer': function(e, value, row) { // 逾期
							$("#confirmTitle").html("确定转让？");
							$$.confirm({
								container: $('#doConfirm'),
								trigger: this,
								accept: function() {
									http.post(config.api.illiquidAsset.transfer, {
										data: {
											oid: row.oid
										},
										contentType: 'form'
			...
			$(document.unsetupForm.buildDate).on('dp.change', function() {
				...
				if(type == 'TARGETTYPE_17' || type == 'TARGETTYPE_18') {
			...
			*/
			...
			function formatTargetData(t) {
				if(t) {
					...
					t2.expAror = t2.expAror ? util.safeCalc(t2.expAror, '*', 100, 2) + '%' : "";
